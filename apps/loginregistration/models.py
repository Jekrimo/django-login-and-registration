...
class Create(models.Manager):
    def register(self,first_name, last_name, email_address, create_password):
        errors = []
        if PASSWORD_REGEX.match(create_password):
            if EMAIL_REGEX.match(email_address):
                ...
                user = Users.objects.create(email = email_address, password = hashed_pass, first_name = first_name, last_name = last_name,)
                return (True, errors)
            else:
